...
if __name__ == '__main__':
    ...
    rate = rospy.Rate(10)
    device = rospy.get_param("speech2text/device", "alsa default")
    silence = rospy.get_param("speec2text/silence", "0 1 00:00:01.0 8%")
    sox = rospy.get_param("speech2text/sox_command", 'sox -b 32 -e floating-point -r 44100 '
                                                     '-t {device} {file} vad silence {silence}')
    get = rospy.get_param("speech2text/google_command", "wget --post-file='{file}' "
                                                        "--header='Content-Type: audio/x-flac; rate=44100;' -O - "
                                                        "'https://www.google.com/speech-api/v2/recognize?output=json&lang=en-us&key={key}'")
    key = rospy.get_param("speech2text/key", "AIzaSyBOti4mM-6x9WDnZIjIeyEU21OpBXqWBgw")

    while not rospy.is_shutdown():
        rec = tempfile.NamedTemporaryFile('w+b', suffix='.flac')
        if subprocess.check_call(sox.format(device=device, file=rec.name, silence=silence), shell=True) == 1:
            rospy.loginfo("sox had an error")
